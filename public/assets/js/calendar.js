...
export const Calendar = (id) => ({
    ...
    renderEvents() {
        if ((!this.events || this.events.length <= 0) && (!this.bookings || this.bookings.length <= 0)) return;
        ...
        lis.forEach((li)=>{
            try {
                ...
                this.bookings.forEach((booking)=>{
                    try {
                        let bookingTime = moment(booking.time);
                        if (bookingTime.year() == y && bookingTime.month() == m && bookingTime.date() == d) {
                            let frgBooking = document.createRange().createContextualFragment(`
                                <div time="${booking.time}" class="booking ${booking.cls}">${bookingTime.format('h:mma')} ${booking.desc}</div>
                            `);
                            divEvents.appendChild(frgBooking);
                            let divBooking = divEvents.querySelector(`.booking[time='${booking.time}']`);
                            divBooking.onclick = this.onEventClick;
                        }
                    } catch (err2) {
                        console.log(err2);
                    }
